...
forwarded receipts directly from a FastmaiFastmail and discards them) adds: `FASTMAIL_TOKEN` (JMAP API token: full mail
...
sender intact (no SRS rewrite, so sender verification works). Note: FastmaiFastmailoffers no API for rules/aliases/domains (the official MCP server is
