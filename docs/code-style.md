...
- Descriptive names, with auxiliary-verb booleans: `isComplete`, `hasAmount`.
- No classes; pure functions only, except for error subclasses (`class DeepSeekError extends Error`).
...
- View transitions (`viewTransition` on `<Form>`/`<Link>`): react-router only starts one for a real navigation, never for a fetcher submission, and the element that should morph needs its own `view-transition-name`. That name is a **stacking context**: a `fixed` dialog rendered inside the named element paints under the elements that follow it and they swallow its clicks (scroll-dependent, so it fails only sometimes) — portal such a dialog to the body, as `app/components/settings/auto-imported-senders.tsx` does. Guard reduced motion for the `::view-transition-*` pseudo-elements explicitly; the global `*` rule cannot reach them.
