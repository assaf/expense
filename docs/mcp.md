...
| ----------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `capture_receipt` | ✓      | Capture a receipt from base64 image/PDF data or a URL; extract merchant/amount/category (reusing the merchant's history), store the image, create the expense. Optional overrides for merchant, amount, currency, category, date, report, description. A non-USD amount converts to USD at the ECB reference rate for the expense date (result carries `fx`, and the description gains a "Converted from …" note); with no rate available the amount is stored as-is and `warning` says so. |
| `log_mileage`     | ✓      | Log a driving trip from ordered stops (address strings or pre-geocoded `{address, lat, lng}`); geocodes, routes, and computes distance + amount at the IRS rate for the trip's date and type (business default). One way by default; pass `roundTrip: true` when the drive returns to its first stop.                                                                                                                                                                                       |
...
| `expense_summary` |        | Totals + per-category breakdown for the same filters; this is the "how much did I spend on X?" tool.                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `get_settings`    |        | Home address, the account's named locations (work, hospital, and so on, each as a name plus its address), and the IRS mileage-rate master table (period + type).                                                                                                                                                                                                                                                                                                                            |
