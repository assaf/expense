...
| `app/routes/settings.tsx`          | Reports, categories, IRS mileage-rate table (read-only master table), home location, receipts-by-email sender, Agents & API (MCP) tokens.                                                                                                                                                                                         |
...
| `app/lib/mileage-rates.ts`         | IRS rate lookup + amount math: `mileageRateFor(entries, date, type)`, `mileageAmount(distance, rate)` (exact half-up cents), `formatRate`, `MILEAGE_TYPES`. Pure — shared by the editor (client) and server.                                                                                                                      |
