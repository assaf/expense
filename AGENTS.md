...
| `app/lib/report-pdf.server.ts`     | Report PDF builder — shared by the web export and the MCP `export_report` tool. Mileage rows show type · rate, distance + route addresses, and an embedded schematic route map (`app/lib/route-map.server.ts`).                                                                                                                   |
...
| `app/lib/route-map.server.ts`      | Schematic route map for report PDFs: draws the saved route geometry (outbound solid, return dashed, numbered stops) to SVG, rasterizes with resvg + bundled JetBrains Mono; straight-line fallback for trips without saved geometry.                                                                                              |
