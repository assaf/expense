...
| `app/lib/sender-verification.server.ts` | Builds + sends the receipts-by-email verification email (from `INBOUND_EMAIL_ADDRESS`; skips with a warning when Resend is unconfigured). Token lifecycle lives in `database.ts` (`addInboundSender` / `verifyInboundSenderAddress` / `ensureInboundSenderForUser`).                                                                                                            |
