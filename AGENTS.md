...
`Receipts`; must match the folder the delivery rule files into), `FASTMAIL_FROM`
(the address replies/verifications are sent FROM, e.g. `receipts@labnotes.org`;
falls back to the account's default identity), and
